package io.github.n1ck145.redhook.core.action;
...
import io.github.n1ck145.redhook.util.common.ColorMapper;
...
public abstract class RedstoneActionBase implements RedstoneAction {
	...
	public ItemStack getIcon() {
		return new ItemBuilder(icon).name(ColorMapper.map(label))
				.lore(description.stream().map(ColorMapper::map).toList()).build();
	}
			...
			for (Field field : actionClass.getDeclaredFields()) {
				...
				if (annotation != null) {
					...
					// Apply color mapping for List<String> fields
						...
						value = list.stream().map(Object::toString).map(ColorMapper::map).toList();
					}
