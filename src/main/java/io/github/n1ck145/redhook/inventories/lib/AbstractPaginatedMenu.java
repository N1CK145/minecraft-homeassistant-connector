...
public abstract class AbstractPaginatedMenu<T> implements Menu {
    ...
    private void addBackground() {
        ...
        ItemStack background = new ItemBuilder(material)
                .name(" ")
                .build();
    ...
    private void addNavigation() {
        // Navigation buttons
        ItemBuilder previousButtonBuilder = new ItemBuilder(menuConfiguration.getPreviousPageItemMaterial())
                .name("§7Previous Page");
        ItemBuilder nextButtonBuilder = new ItemBuilder(menuConfiguration.getPreviousPageItemMaterial())
                .name("§7Next Page");

        // Add page indicator
        ItemBuilder pageIndicatorBuilder = new ItemBuilder(Material.COMPASS)
                .name("§7Page " + (currentPage + 1) + " / " + (getLastPage() + 1))
                .addGlint();
